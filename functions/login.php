<?php
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        if($username && $password){
            ...
            $sql = "SELECT user_id, full_name, email, phone, type, image FROM users WHERE username = '$username' AND password = md5('$password')";
            $user = $db->query($sql);
            ...
            if($user && @mysqli_num_rows($user) == 1){
                ...
                $_SESSION = mysqli_fetch_array ($user, MYSQLI_ASSOC);
